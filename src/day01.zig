...
pub fn day01a(allocator: std.mem.Allocator, file: []const u8) ![]const u8 {
    ...
    var curr: u16 = try std.fmt.parseInt(u16, line_iterator.first(), 10);
    var up_counter: u16 = 0;
    while (line_iterator.next()) |line| {
        if (line.len == 0) {
            break;
        }
        const next: u16 = try std.fmt.parseInt(u16, line, 10);
        if (next > curr) {
            up_counter += 1;
        }
        curr = next;
...
pub fn day01b(allocator: std.mem.Allocator, file: []const u8) ![]const u8 {
    ...
    var sums = std.ArrayList(u16).init(allocator);
    defer sums.deinit();
